...
def optimize(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, key, addconstraints=[None,None], max_iters=10**5):
    """
    Parameters
    ----------
    ...
    if addconstraints[0] == 'Sensor Norm' or addconstraints[0] == 'Reweighted Sensor Norm':
        constraints += [SLS_data.Phi_uy[:, addconstraints[1]] == 0]
    elif addconstraints[0] == 'Actuator Norm' or addconstraints[0] == 'Reweighted Actuator Norm':
        constraints += [SLS_data.Phi_uy[addconstraints[1], :] == 0]
    # objective function
    if key == 'Feasibility':
    ...
    #print(problem.is_qp())
    result = problem.solve(solver=cp.SCS, verbose=True,max_iters=max_iters)
    return result, SLS_data, Lambda, problem.status

def optimize_RTH(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, N, key):
    """
    Parameters
    ----------
    A_list: list of matrices [A_0, ...A_T]
    ...
    # objective
    if key == 'Reweighted Feasibility':
        objective = cp.Minimize(0)
    elif key == 'Reweighted Nuclear Norm':
        W_1XW_2 = W_1 @ SLS_data.Phi_uy @ W_2
        min_var = cp.Variable(W_1XW_2.shape)
        objective = cp.Minimize(cp.norm(min_var, 'nuc'))
        constraints += [min_var == W_1XW_2]
    else:
        raise Exception('Choose key: \'Feasibility\' or \'Nuclear Norm\'.')
    #define problem
    problem = cp.Problem(objective, constraints)
    for k in range(N):
        ...
        #update params
        [U, S, Vh] = np.linalg.svd(W_1XW_2.value)
        Y = np.linalg.inv(W_1.value).dot(W_1XW_2.value).dot(Vh.T).dot(U.T).dot(np.linalg.inv(W_1.value))
        Z = np.linalg.inv(W_2.value).dot(Vh.T).dot(U.T).dot(W_1XW_2.value).dot(np.linalg.inv(W_2.value))
        # help function
...
def optimize_reweighted_atomic(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, N, key):
    SLS_data = SLSFinite(A_list, B_list, C_list)
    [constraints, Lambda] = polytope_constraints(SLS_data, Poly_x, Poly_u, Poly_w)
    # Initialize Paramters
    delta = 0.01
...
def optimize_sparsity(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, key, N, epsilon):
    """
    Parameters
    ----------
    A_list: list of matrices [A_0, ...A_T]
    ...
    Lambda: cvxpy.Variable, shape (H_x[0] + H_u[0], H_w[0])
        Polytope containment variable corresponding to the optimal cost.
    """
    if key == 'Actuator Norm' or key == 'Sensor Norm':
        result, SLS, Lambda, status = optimize(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, key)
        if status != cp.OPTIMAL:
            raise Exception('Atomic norm minimization did not converge.')
    if key == 'Reweighted Actuator Norm' or key == 'Reweighted Sensor Norm':
    ...
    if key == 'Sensor Norm':
        ...
        norm_list = [norm_argmin]
    elif key == 'Reweighted Sensor Norm':
        norm_argmin = np.linalg.norm(argmin, 2, 0)
    elif key == 'Actuator Norm':
        ...
        raise Exception('Choose either the sensor or actuator norm to minimize!')
    ...
    #assert len(ind) == i+1
    # define new variable
    test_result, test_SLS, _, status = optimize(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, 'Feasibility', addconstraints=[key, ind], max_iters = 1000000)
    #if status != cp.OPTIMAL:
    #    break
...
def causal_rank_decomposition(F, nu, ny, T):
    ...
    for t in range(T+1):
        for s in range(nu):
            ...
            rank_new = np.linalg.matrix_rank(submat_new, tol=1e-5)
    ...
    assert rank_counter == np.linalg.matrix_rank(F, tol=1e-5)
...
if __name__ == '__main__':
    ...
    # define w polytope
    # used for T = 40. Discretized drone, max_v = 0.1, u_scale = 0.1, w_scale = 0.005
    w_scale = T/300
    v_scale = T/300
    ...
    save = True
    direc = 'T20_1/'
    #result, SLS, Lambda, status = optimize(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, key); result_list = [result]; SLS_list = [SLS]
    #result_list, SLS_list, Lambda, status = optimize_RTH(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, 7, key)
    #for key in ['Reweighted Sensor Norm', 'Reweighted Actuator Norm']:
    result_list, SLS_list, norm_list, reopt_ind_list, Lambda = optimize_sparsity(A_list, B_list, C_list, Poly_x, Poly_u, Poly_w, key, 7, 1e-4)
    ...
    for i in range(N):
        w = np.concatenate([w_scale*np.random.uniform(-1, 1, (T+1)*(SLS.nx)), v_scale*np.random.uniform(-1, 1, (T+1)*(SLS.ny))])
        ...
        #u statistics
        u_trajs[i,:] = np.block([SLS.Phi_ux.value, SLS.Phi_uy.value]) @ w
    ...
    #F = SLS.F
    epsilon = 10**-4
    ax2.spy(SLS.Phi_uy.value, precision=epsilon)
    title2 = 'Sparsiy of $\mathbf{\Phi}_{uy}$ - ' + key
    save2 = 'Sparsity of Phi_uy - ' + key
    ...
    if key == 'Nuclear Norm' or key == 'Reweighted Nuclear Norm':
        ...
        D, E, rank_F = causal_rank_decomposition(SLS.F, SLS.nu, SLS.ny, SLS.T)
        ...
        axs[1].spy(E, 1e-5)
        F_trunc = D.dot(E)
        Phi_trunc = SLS.F_to_Phi(F_trunc)
        if save:
    ...
    #Poly_xu = Poly_x.cart(Poly_u)
    # original solution
    #assert np.all( np.isclose( (Lambda.value.dot(Poly_w.H)).astype('float'), (Poly_xu.H.dot(SLS.Phi_matrix.value)).astype('float') , atol = 1e-4) )
