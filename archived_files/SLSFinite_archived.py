...
class SLSFinite():
    def __init__(self, A_list, B_list, C_list, norm=None):
        ...
        if norm == None:
            ...
            elif norm[0] == 'Reweighted Nuclear Norm':
                self.Phi_uy = cp.Parameter([self.nu*(self.T+1), self.ny*(self.T+1)])
                self.Phi_uy.value = norm[1]
                self.Phi_ux = low_block_tri_variable(self.nu, self.nx, self.T+1)
                self.Phi_xy = low_block_tri_variable(self.nx, self.ny, self.T+1)
            else:
                raise Exception("row or col!")
            
        #self.Phi_xx = cp.Variable((self.nx*(self.T+1), self.nx*(self.T+1)))
        #self.Phi_xy = cp.Variable((self.nx*(self.T+1), self.ny*(self.T+1)))
        #self.Phi_ux = cp.Variable((self.nu*(self.T+1), self.nx*(self.T+1)))
        #self.Phi_uy = cp.Variable((self.nu*(self.T+1), self.ny*(self.T+1)))
    ...
    def calculate_dependent_variables(self):
        ...
        """
        self.F_sum = self.Phi_uy.value - self.Phi_ux.value @ np.linalg.inv((self.Phi_xx.value).astype('float64')) @ self.Phi_xy.value
        self.F = np.linalg.inv( (np.eye(self.nu*(self.T+1)) + self.Phi_ux.value @ self.Z @ self.cal_B).astype('float64') ) @ self.Phi_uy.value
        assert np.all(np.isclose( self.F.astype('float64'), self.F_sum.astype('float64')) )
        return
    ...
    def causal_rank_decomposition(self, rank_eps, matrix_key, method=None):
        if matrix_key == 'F':
            low_btri = self.F
        elif matrix_key == 'Phi':
            low_btri = self.Phi_uy.value
        ...
        for t in range(self.T+1):
            for s in range(self.nu):
                ...
                if method == None:
                    ...
                    try_row = low_btri[row, :].reshape((1,-1))
                    try_E = sympy.Matrix(np.hstack([E.T, try_row.T]))
                    rank_new = len(try_E.rref()[1])
                else:
                ...
                if rank_new - rank_counter == 1:
                    ...
                    #c = np.linalg.solve(E[:,0:rank_counter].T, low_btri[row, 0:rank_counter])
        ...
        if matrix_key == 'F':
            ...
            self.E_phi = E
            self.D_phi = D
            self.Phi_uy_trunc = D.dot(E)
            self.rank_Phi_uy_trunc = rank_low_btri
        return
    
    def causal_rank_factorization(self, rank_eps, matrix_key):
